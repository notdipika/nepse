/**
 * GET /api/stocks/all
 * Returns all active companies for the MOST RECENT date that has data.
 * If today has data → shows today. If market closed → shows last trading day.
 */
...
// Cache latest date for 60s so every component re-render doesn't re-query
...
export async function GET() {
  try {
    ...
    if (!latestDate) {
      return NextResponse.json({ rows: [], tradingDate: null })
    }
