...
interface Row {
  ...
  close_price: number; prev_close: number; volume: number
  turnover: number; percent_change: number
}
...
function SqlPanel({ sql, count, ms }: { sql: string; count: number; ms: number }) {
  ...
  const hl = sql
    .replace(/\b(SELECT|FROM|JOIN|LEFT JOIN|WHERE|AND|OR|ORDER BY|LIMIT|ON|AS|COUNT|AVG|SUM|MAX|MIN|GROUP BY|HAVING|DISTINCT|DATE_FORMAT|COALESCE|ROUND|NULLIF)\b/g,
      '<span style="color:#2563eb;font-weight:600">$1</span>')
    .replace(/\b(price_data|company|trading_session|sector|watchlist)\b/g,
      '<span style="color:#7c3aed">$1</span>')
    .replace(/\b(pd|c|ts|s)\b(?=\s*\.)/g, '<span style="color:#7c3aed">$1</span>')
  ...
  return (
    <div className="bg-white rounded-xl border overflow-hidden" style={{ borderColor: '#e2e8f0' }}>
      <div className="flex items-center justify-between px-4 py-3 border-b"
        ...
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-bold px-2 py-0.5 rounded border"
            style={{ color: '#2563eb', background: '#eff6ff', borderColor: '#bfdbfe', fontFamily: 'monospace' }}>SQL</span>
          <span className="text-xs" style={{ color: '#64748b' }}>
            {count} rows · {ms}ms · sent to MySQL
          </span>
        </div>
        <button onClick={copy} className="text-xs px-2.5 py-1 rounded border font-medium transition-all"
          style={{
      ...
      <pre className="p-4 text-xs leading-relaxed overflow-x-auto"
        style={{ maxHeight: 300, background: '#f8fafc', color: '#374151', fontFamily: 'monospace' }}
    ...
    <div className="bg-white border rounded-lg p-2.5 text-xs shadow-md" style={{ borderColor: '#e2e8f0' }}>
      ...
      {payload.map((p: any) => (
        <div key={p.name} className="flex justify-between gap-4">
          ...
          <span className="font-medium" style={{ color: '#0f172a', fontFamily: 'monospace' }}>
...
export default function AnalyticsPage() {
  ...
  // Load companies + sectors on mount
  useEffect(() => {
    Promise.all([
  ...
  // Auto-run on filter change (debounced 400ms)
  useEffect(() => {
    const t = setTimeout(runQuery, 400)
  ...
  const chartData = symbol !== 'all'
    ? [...data].sort((a, b) => a.date.localeCompare(b.date)).slice(-60)
    : data.slice(0, 20)
    ...
    <div className="space-y-5 max-w-screen-xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: '#64748b' }}>
          Filter NEPSE data by any combination — the exact MySQL query is shown live below
        </p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
          ...
          <div style={{ gridColumn: 'span 2' }}>
            ...
            <select value={symbol} onChange={e => setSymbol(e.target.value)} style={inp}>
              ...
              {companies.map(c => (
                <option key={c.symbol} value={c.symbol}>{c.symbol} — {c.name.slice(0, 24)}</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input type="number" value={minTurn} onChange={e => setMinTurn(e.target.value)}
          ...
          <div>
            ...
            <input type="number" step="0.1" value={minChg} onChange={e => setMinChg(e.target.value)}
          ...
          <div>
            ...
            <input type="number" step="0.1" value={maxChg} onChange={e => setMaxChg(e.target.value)}
        ...
        <div className="bg-white rounded-xl border p-5" style={{ borderColor: '#e2e8f0' }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold" style={{ color: '#0f172a' }}>
              {symbol !== 'all' ? `${symbol} — Price Chart` : 'Top 20 Companies'}
            </h2>
            <div className="flex gap-2">
              {(['price','volume'] as const).map(ct => (
                <button key={ct} onClick={() => setChart(ct)}
                  className="text-xs px-2.5 py-1 rounded-md border font-medium transition-all"
                  style={{
                    background:  chart === ct ? '#eff6ff' : 'white',
                    color:       chart === ct ? '#2563eb' : '#64748b',
                    borderColor: chart === ct ? '#bfdbfe' : '#e2e8f0',
                    cursor: 'pointer',
                  }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            {chart === 'price' ? (
              <AreaChart data={chartData} margin={{ top:4, right:4, left:0, bottom:0 }}>
                ...
                <XAxis dataKey={symbol !== 'all' ? 'date' : 'symbol'}
                  tick={{ fill:'#94a3b8', fontSize:10 }} tickLine={false} axisLine={false} interval="preserveStartEnd"/>
                <YAxis tick={{ fill:'#94a3b8', fontSize:10 }} tickLine={false} axisLine={false}
                  tickFormatter={v => `Rs.${v}`} width={75}/>
                <Tooltip content={<CTip/>}/>
                <Area type="monotone" dataKey="close_price" name="Close"
                  stroke="#2563eb" strokeWidth={2} fill="url(#blueGrad)" dot={false}/>
                {symbol !== 'all' && (
                  <Area type="monotone" dataKey="open_price" name="Open"
                    stroke="#93c5fd" strokeWidth={1} fill="none" dot={false} strokeDasharray="4 4"/>
                )}
              </AreaChart>
            ) : (
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={11} className="text-center py-10" style={{ color: '#94a3b8' }}>
                ...
                return (
                  <tr key={i} className="border-b transition-colors cursor-pointer"
                    ...
                    <td className="px-4 py-2.5 max-w-[140px] truncate" style={{ color: '#475569' }}>{row.name}</td>
                    <td className="px-4 py-2.5">
                      <span className="px-1.5 py-0.5 rounded"
                        style={{ background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', fontSize: 10 }}>
                    ...
                    <td className="px-4 py-2.5">
                      <span className="px-1.5 py-0.5 rounded font-semibold"
                        ...
                        {up ? '+' : ''}{(row.percent_change || 0).toFixed(2)}%
                      </span>
                    </td>
